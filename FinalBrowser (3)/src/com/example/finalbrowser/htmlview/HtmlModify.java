...
public class HtmlModify extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		while((html_src= ((MainActivity) MainActivity.mContext).html).split("</head>").length<2){
			try {
				((MainActivity) MainActivity.mContext).getWeb().loadUrl("javascript:console.log(document.documentElement.innerHTML)");
				wait(200);
		...
		System.out.println("�����0 : " + html_src.length());
		System.out.println("�����1 : " + host_name);
		...
		btConfirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				((MainActivity) MainActivity.mContext).temp_repo = temp_head + htmlmd.getText()
						.toString();
				((HtmlModify)mContext).setResult(RESULT_OK);
	...
	public void search(String word){
		if(htmlmd.getText().toString().contains(word)){
			...
			if(index<0){
				Toast.makeText(this, "�� �̻� ���ڰ� �����ϴ�",Toast.LENGTH_SHORT).show();
				return;
			}
